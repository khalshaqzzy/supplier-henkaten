...
Status: active · Updated: 2026-09-24 · Approach: backend first, dependency driven
...
The [QA report](../docs/audits/local-browser-qa-2026-09-24.md) is the finding ledger. Reproduce, fix or explicitly disposition each of **F-01–F-12** and rerun its affected journey. The highest-priority observed blocker is **F-06 (High)**: reverse External→Hosted Preparation fails after a cutover. **F-01, F-05, F-08 (Medium)** affect board photos, Supplier Admin reset/replacement, and post-edit member/part actions. The remaining Low defects and product risks include clone validity, successful-login throttling, stale notification UI, warning identifier, copied Line–Shift selection, reroute notification/action, and registry pagination. F-10 needs a product decision on reroute notification expectation. None is marked fixed by the QA run.
