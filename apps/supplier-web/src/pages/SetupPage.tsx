      ...
      {readiness.data && (
        <>
          <section className="setup-summary">
            <div className={readiness.data.ready ? 'is-ready' : undefined}>
              {readiness.data.ready ? <Check /> : <CircleDashed />}
              <span>
                <strong>
                  {readiness.data.ready ? 'Siap beroperasi' : 'Konfigurasi belum lengkap'}
                </strong>
                <small>
                  {readiness.data.ready
                    ? 'Semua prerequisite Hosted telah dipenuhi.'
                    : `${readiness.data.blockers.length} blocker masih perlu diselesaikan.`}
                </small>
              </span>
            </div>
            <LastUpdated
          ...
          {readiness.data.blockers.length > 0 && (
            <Panel title="Blocker saat ini" description="Hasil evaluator readiness authoritative.">
